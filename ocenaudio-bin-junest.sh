...
case "$1" in
	'') $HERE/.local/share/junest/bin/junest -n -b "$ETC_RESOLV" 2> /dev/null -- /usr/share/ocenaudio/bin/ocenaudio "$@";;
	*) $HERE/.local/share/junest/bin/junest -n -b "$ETC_RESOLV" 2> /dev/null -- /usr/share/ocenaudio/bin/ocenvst "$@";;
esac
EOF
chmod a+x ./AppRun
...
mv ./*AppImage ./"$(cat ./$APP.AppDir/*.desktop | grep 'Name=' | head -1 | cut -c 6- | sed 's/ /-/g')"-junest_"$VERSION"-archimage3.1-x86_64.AppImage
