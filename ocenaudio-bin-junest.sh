...
APP=ocenaudio-bin
BIN="ocenaudio" #CHANGE THIS IF THE NAME OF THE BINARY IS DIFFERENT FROM "$APP" (for example, the binary of "obs-studio" is "obs")
...
SHARESAVED="icons"
LIBSAVED="libpxbackend libogg.so libvorbisenc.so libFLAC.so libmpg123.so libmp3lame.so"
...
_post_installation_processes() {
	printf "\n◆ User's processes: \n\n"
	rsync -av base/opt/ocenaudio/* AppDir/.junest/usr/lib/ocenaudio/ || exit 1
	rm -Rf AppDir/.junest/opt/*
}
...
case "$1" in
	'') _JUNEST_CMD -- /usr/lib/ocenaudio/bin/ocenaudio "$@";;
	*) _JUNEST_CMD -- /usr/lib/ocenaudio/bin/ocenvst "$@";;
esac
