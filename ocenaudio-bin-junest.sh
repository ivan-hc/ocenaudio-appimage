...
LIBSAVED="libpxbackend libogg.so libvorbisenc.so libFLAC.so libmpg123.so libmp3lame.so libnghttp libidn libssh libpsl.so libxcb qt $lib_audio_keywords $lib_browser_launcher"
...
case "$1" in
	'') _JUNEST_CMD -- /usr/share/ocenaudio/bin/ocenaudio "$@";;
	*) _JUNEST_CMD -- /usr/share/ocenaudio/bin/ocenvst "$@";;
esac
