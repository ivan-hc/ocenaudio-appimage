...
sed -i 's#--bind "$HOME" "$HOME"#--bind /opt /opt --bind /usr/lib/locale /usr/lib/locale --bind /usr/share/fonts /usr/share/fonts --bind /usr/share/themes /usr/share/themes --bind /mnt /mnt --bind /media /media --bind /home /home --bind-try /run/user /run/user#g' .local/share/junest/lib/core/namespace.sh
...
SHARESAVED="certificates $APP" # Enter here keywords or file/folder names to save in both /usr/share and /usr/lib
...
mv ./*AppImage ./"$(cat ./$APP.AppDir/*.desktop | grep 'Name=' | head -1 | cut -c 6- | sed 's/ /-/g')"-junest_"$VERSION"-archimage3.2-x86_64.AppImage
